...
void IOROS::recvState(LowlevelState *state){
    ...
    constexpr std::uint64_t maxContactAgeUs = 1000000ULL;
    for(int i=0; i<4; ++i){
        const std::uint64_t stampNs = _foot_force_wall_stamp_ns[i].load();
        const std::uint64_t forceStampUs = _foot_force_sim_time_us[i].load(
            std::memory_order_relaxed);
        state->footForce[i] = _foot_force[i].load();
        const bool simFresh = forceStampUs != 0 && stampUs >= forceStampUs &&
            (stampUs - forceStampUs) <= maxContactAgeUs;
        const bool wallFresh = stampNs != 0 && nowNs >= stampNs &&
            (nowNs - stampNs) <= maxContactAgeNs;
        // Gazebo can run far below real time while DSV is planning.  Prefer
        // simulation age so a current contact sample does not expire merely
        // because more than one wall-clock second elapsed between callbacks.
        state->footForceValid[i] = forceStampUs != 0 ? simFresh : wallFresh;
        state->footForceCallbackSequence[i] = _foot_force_callback_sequence[i].load(
            std::memory_order_relaxed);
        state->footForceSimTimeUs[i] = forceStampUs;
    }
...
void IOROS::updateFootForce(int index, const geometry_msgs::WrenchStamped& msg){
    ...
    const std::uint64_t messageStampUs =
        static_cast<std::uint64_t>(msg.header.stamp.toNSec() / 1000ULL);
    // The Gazebo foot-contact plugin publishes a zero header stamp.  In that
    // case snapshot the latest /clock value so contact freshness stays on the
    // same simulation-time basis as recvState(), even at a very low RTF.
    const std::uint64_t contactStampUs = messageStampUs != 0
        ? messageStampUs
        : current_time.load(std::memory_order_acquire);
    _foot_force_sim_time_us[index].store(contactStampUs,
                                        std::memory_order_relaxed);
}
