...
COMMAND_VX="${COMMAND_VX:?set COMMAND_VX, e.g. 0.10}"
RUN_ID="${RUN_ID:?set RUN_ID, e.g. vx_010_run_01}"
VX_TAG="${VX_TAG:-$(printf 'vx_%03d' "$(awk "BEGIN {printf \"%d\", ${COMMAND_VX} * 100}")")}"
OUT="$RUN_ROOT/baseline/$VX_TAG/$RUN_ID"
ROS_PORT="${ROS_PORT:-}"
if [ -z "$ROS_PORT" ]; then
  ROS_PORT="$(awk -v vx="$COMMAND_VX" -v id="$RUN_ID" 'BEGIN {
    gsub(/[^0-9]/, "", id); suffix = int(id) % 100;
    printf "%d", 12000 + int(vx * 1000) + suffix
  }')"
fi
...
cleanup() {
  ...
  tmux kill-session -t "simenv-g2-${RUN_ID}-junior_ctrl" 2>/dev/null || true
...
if [ ! -f "$WORKSPACE/devel/setup.bash" ]; then
  echo "Missing $WORKSPACE/devel/setup.bash; build before running G2 trials." >&2
  exit 2
fi
...
TMUX_SESSION_PREFIX="simenv-g2-${RUN_ID}" \
...
timeout --signal=TERM --kill-after=10s 360s \
  /usr/bin/python3 "$RUN_ROOT/g2_capture_trial.py" \
...
tmux capture-pane -t "simenv-g2-${RUN_ID}-junior_ctrl" -p -S -3000 \
