...
def main() -> int:
    ...
    parser.add_argument("--case-kind", choices=("g0", "g1", "g1p", "g1s", "fixedstand", "rl"), required=True)
    ...
    if args.case_kind != "g1p" and not wait_until(lambda: cap.clock_rows and rospy.Time.now().to_sec() > 0.0, 60.0):
    ...
    elif args.case_kind in ("g1", "g1s"):
        if not wait_until(lambda: cap.pose_rows, 60.0):
            reasons.append("NO_ROBOT_POSE")
        ok = wait_sim_duration(args.duration, args.wall_timeout)
    elif args.case_kind == "g1p":
        ok = wait_until(lambda: cap.pose_rows, 60.0)
        if not ok:
            reasons.append("NO_ROBOT_POSE")
    else:
    ...
    if args.case_kind == "fixedstand":
        ...
        verdict = "G1P_PASS" if pose_summary.get("valid") and not reasons else "G1P_FAIL_GEOMETRY"
    elif args.case_kind in ("g1", "g1s"):
        verdict = metrics.classify_fixedstand(pose_summary)
        verdict = "G1S_PASS" if verdict == "PASS" and not reasons else "G1S_FAIL_ATTITUDE"
    else:
    ...
    return 0 if result["verdict"] in ("G0_PASS", "G1P_PASS", "G1S_PASS", "PASS") or result["verdict"].startswith("FAIL") else 3
