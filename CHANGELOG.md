...
- **Camera init TF bridge orientation**: Iterated through several orientation fixes for the `map→camera_init` static TF: (1) switched from `laser_livox` to `imu_link` to remove 45° LiDAR tilt, (2) reverted a 180° Z-flip that broke odometry, (3) settled on `imu_link + Ry(-45°)` to match LiDAR scan-data output convention while keeping odometry axes correct.
