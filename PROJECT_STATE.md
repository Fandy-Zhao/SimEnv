...
- **Build/startup recovery**: the supported build script now compiles the complete `auto.sh` runtime profile while excluding unrelated locally added packages (including legacy PS3 utilities requiring libusb-0.1); `auto.sh` checks for `junior_ctrl` before it mutates the running simulation state.
