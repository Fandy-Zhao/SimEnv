...
- Current focus: FSM nullptr segfault fix on Torch-disabled builds
...
- **Dedicated controller + rviz terminals**: `auto.sh` launches `junior_ctrl` and rviz in dedicated terminal windows (`gnome-terminal.real`/`xterm`/background fallback). It sanitises only the terminal-launcher environment to prevent Snap Code library contamination from blocking GNOME Terminal, while preserving the ROS environment inside each terminal. This guarantees a real TTY for keyboard input. Removed `CONTROLLER_FOREGROUND`; added `ENABLE_RVIZ`.
